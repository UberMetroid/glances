...
#[test]
fn pub_has_no_reply_to_field() {
    // Regression: a stray "0" field was emitted as the reply-to
    // subject. The frame must have exactly 2 tokens after PUB.
    let cmd = nats::build_pub("glances", "cpu", "total", b"42");
    let raw = String::from_utf8(cmd).unwrap();
    let head = raw.split("\r\n").next().unwrap();
...
fn subject_sanitizes_wildcards_and_whitespace() {
    // ' ', '*', '>' are illegal/protocol-active in NATS subjects.
    let cmd = nats::build_pub("g l", "cp*u", "to>tal", b"1");
    let raw = String::from_utf8(cmd).unwrap();
...
fn publish_set_emits_one_pub_per_field() {
    ...
    let bytes = nats::build_publishes(&flat(&snap), "gl");
...
fn empty_subject_prefix_rejected() {
    ...
    let err = nats::write(&flat(&snap), &cfg).unwrap_err();
